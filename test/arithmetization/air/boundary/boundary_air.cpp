#include <nil/crypto3/zk/snark/arithmetization/air/boundary/air.hpp>
    ...
    namespace {
        ...
        */
        TEST(BoundaryAir, Correctness) {
            ...
            const uint64_t actual_degree = ComputeCompositionDegree(air, Trace(std::move(trace)), random_coefficients);
        ...
        */
        TEST(BoundaryAir, Soundeness) {
            ...
            for (size_t condition_index = 0; condition_index < n_conditions; ++condition_index) {
                ...
                boundary_conditions.emplace_back(column_index, ExtensionFieldElement::RandomElement(&prng),
