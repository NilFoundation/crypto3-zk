...
namespace starkware {
    ...
    */
    ...
    public:
        CompositionOracleProver(MaybeOwnedPtr<const EvaluationDomain> evaluation_domain,
                                ...
                                MaybeOwnedPtr<const CompositionPolynomial> composition_polynomial,
        ...
        */
        void EvalMaskAtPoint(const ExtensionFieldElement &point, gsl::span<ExtensionFieldElement> output) const;
        ...
        */
        ...
        const EvaluationDomain &GetEvaluationDomain() const {
            return *evaluation_domain_;
        }

        gsl::span<const std::pair<int64_t, uint64_t>> GetMask() const {
            return mask_;
        }
        ...
        */
        ...
        MaybeOwnedPtr<CommittedTraceProverBase<BaseFieldElement>> MoveTrace() && {
            ASSERT_RELEASE(!composition_trace_.HasValue(),
    ...
    public:
        ...
        std::vector<ExtensionFieldElement> VerifyDecommitment(std::vector<std::pair<uint64_t, uint64_t>> queries) const;
        ...
        */
        ...
        const CompositionPolynomial &GetCompositionPolynomial() const {
            return *composition_polynomial_;
        }
    ...
    namespace {
        ...
        */
        ...
        /*
          ...
          The split masks will be:
            ...
            {(20,2), (40,1), (50,1), (60,3)}.
        */
        std::vector<std::vector<std::pair<int64_t, uint64_t>>>
            SplitMask(gsl::span<const std::pair<int64_t, uint64_t>> mask, const std::vector<uint64_t> &widths) {
        ...
        */
        std::vector<std::tuple<uint64_t, uint64_t, size_t>>
            QueriesToTraceQueries(gsl::span<const std::pair<uint64_t, uint64_t>> queries,
            ...
            for (const auto &[coset_index, offset] : queries) {
                for (const auto &[mask_row, mask_col] : trace_mask) {
                    ...
                    const uint64_t new_offset =
                        BitReverse((BitReverse(offset, n_trace_bits) + mask_row) & (trace_length - 1), n_trace_bits);
    ...
    CompositionOracleProver::CompositionOracleProver(
        ...
        trace_(std::move(trace)), composition_trace_(std::move(composition_trace)),
        evaluation_domain_(std::move(evaluation_domain)), air_(std::move(air)),
        composition_polynomial_(std::move(composition_polynomial)), channel_(channel) {
    ...
    std::vector<ExtensionFieldElement> CompositionOracleProver::EvalComposition(uint64_t task_size,
                                                                                uint64_t n_cosets) const {
        const uint64_t trace_length = evaluation_domain_->TraceSize();
        const size_t n_segments = n_cosets;
        ASSERT_RELEASE(n_segments <= evaluation_domain_->NumCosets(),
        ...
        if (composition_trace_.HasValue() && !composition_trace_->GetLde()->IsEvalNaturallyOrdered()) {
            ...
            for (size_t i = 0; i < trace_widths_[1]; ++i) {
                composition_tmp_storages.emplace_back(ExtensionFieldElement::UninitializedVector(trace_length));
            }
        }
            ...
            auto eval_trace = [&](const auto &trace, auto *eval_vec, auto *tmp_storages) {
                ...
                for (auto &coset_column_eval : *coset_columns_eval) {
                    if (trace->GetLde()->IsEvalNaturallyOrdered()) {
                        ...
                        ASSERT_RELEASE(tmp_storage_index < tmp_storages->size(), "Not enough temporary storages.");
                        BitReverseVector(gsl::make_span(coset_column_eval),
    ...
    void CompositionOracleProver::DecommitQueries(const std::vector<std::pair<uint64_t, uint64_t>> &queries) const {
        {
    ...
    void CompositionOracleProver::EvalMaskAtPoint(const ExtensionFieldElement &point,
    ...
    size_t CompositionOracleProver::Width() const {
        return Sum(trace_widths_);
    }
    ...
    CompositionOracleVerifier::CompositionOracleVerifier(
        ...
        composition_trace_(std::move(composition_trace)), mask_(mask.begin(), mask.end()),
        evaluation_domain_(std::move(evaluation_domain)), air_(std::move(air)),
        composition_polynomial_(std::move(composition_polynomial)), channel_(channel) {
        trace_widths_.push_back(trace_->NumColumns());
        if (composition_trace_.HasValue()) {
    ...
    std::vector<ExtensionFieldElement>
        CompositionOracleVerifier::VerifyDecommitment(std::vector<std::pair<uint64_t, uint64_t>> queries) const {
        ...
        for (const auto &[coset_index, offset] : queries) {
            ...
            auto neighbors =
                gsl::make_span(trace_mask_values).subspan(splitted_masks_[0].size() * i, splitted_masks_[0].size());
            auto composition_neighbors = composition_trace_.HasValue() ?
            ...
            oracle_evaluations.push_back(composition_polynomial_->EvalAtPoint(point, neighbors, composition_neighbors));
        }
    ...
    size_t CompositionOracleVerifier::Width() const {
        return Sum(trace_widths_);
    }
