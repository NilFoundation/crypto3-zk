#include "nil/crypto3/zk/snark/systems/stark/stark.hpp"
...
#include <nil/crypto3/zk/snark/systems/stark/utils.hpp>
...
namespace starkware {
    namespace {
        ...
        std::unique_ptr<TableProver<FieldElementT>> MakeTableProver(uint64_t n_segments, uint64_t n_rows_per_segment,
        ...
        template<typename FieldElementT>
        std::unique_ptr<TableVerifier<FieldElementT>> MakeTableVerifier(uint64_t n_rows, uint64_t n_columns,
        ...
        std::vector<size_t> DrawFriStepsList(const size_t log_degree_bound, Prng *prng) {
            ...
            ASSERT_RELEASE(Sum(res) == log_degree_bound, "The sum of all steps must be the log of the degree bound.");
            return res;
        }

        StarkParameters GenerateParameters(std::unique_ptr<Air> air, Prng *prng, size_t proof_of_work_bits = 15) {
            const size_t trace_length = air->TraceLength();
            uint64_t degree_bound = air->GetCompositionPolynomialDegreeBound() / trace_length;

            const auto log_n_cosets = (prng == nullptr ? 4 : prng->UniformInt<size_t>(Log2Ceil(degree_bound), 6));
            ...
            return StarkParameters(n_cosets, trace_length, TakeOwnershipFrom(std::move(air)),
        ...
        public:
            explicit StarkTest(bool use_random_values = true) :
                ...
                stark_config(StarkProverConfig::Default()), prover_channel(ProverChannel(channel_prng.Clone())) {
                base_table_prover_factory = [this](uint64_t n_segments, uint64_t n_rows_per_segment, size_t n_columns) {
                    return MakeTableProver<BaseFieldElement>(n_segments, n_rows_per_segment, n_columns,
                                                             &prover_channel);
                };
                extension_table_prover_factory = [this](uint64_t n_segments, uint64_t n_rows_per_segment,
                                                        size_t n_columns) {
                    return MakeTableProver<ExtensionFieldElement>(n_segments, n_rows_per_segment, n_columns,
                                                                  &prover_channel);
                };
            }

            bool VerifyProof(const std::vector<std::byte> &proof,
                ...
                StarkVerifier stark_verifier(UseOwned(&verifier_channel), UseOwned(&base_table_verifier_factory),
        ...
        public:
            explicit TestAirStarkTest(bool use_random_values = true) :
                StarkTest(use_random_values), secret(BaseFieldElement::RandomElement(&(this->prng))),
                claimed_res(TestAir::PublicInputFromPrivateInput(secret, res_claim_index)),
                stark_params(
                    GenerateParameters(std::make_unique<TestAir>(this->trace_length, res_claim_index, claimed_res),
                                       use_random_values ? &(this->prng) : nullptr, 15)) {
            }

            const StarkParameters &GetStarkParams() override {
                return stark_params;
            }

            std::vector<std::byte> GenerateProof() {
                return GenerateProofWithAnnotations().first;
            }
            ...
            std::pair<std::vector<std::byte>, std::vector<std::string>> GenerateProofWithAnnotations() {
                ...
                StarkProver stark_prover(UseOwned(&(this->prover_channel)),
                                         UseOwned(&(this->base_table_prover_factory)),
            ...
            void SetAir(TestAir air) {
                stark_params.air = UseMovedValue<TestAir>(std::move(air));
            }
        ...
        TEST_F(TestAirStarkTest, StarkWithFriProverBadTrace) {
            ...
            trace.SetTraceElementForTesting(bad_column, bad_index,
                                            trace.GetColumn(bad_column).at(bad_index) + BaseFieldElement::One());

            // Send the corrupted trace to a STARK prover.
            StarkProver stark_prover(UseOwned(&this->prover_channel), UseOwned(&this->base_table_prover_factory),
                                     UseOwned(&this->extension_table_prover_factory),
        ...
        TEST_F(TestAirStarkTest, StarkWithFriProverPublicInputInconsistentWithWitness) {
            ...
            // Bad claimed element.
            this->stark_params =
                GenerateParameters(std::make_unique<TestAir>(this->trace_length, this->res_claim_index,
                                                             this->claimed_res + BaseFieldElement::One()),
                                   &(this->prng));
            ...
            stark_prover.ProveStark(TestAir::GetTrace(this->secret, this->trace_length, this->res_claim_index));
            EXPECT_FALSE(this->VerifyProof(this->prover_channel.GetProof()));
        }
        ...
        TEST_F(TestAirStarkTest, ChangeProofContent) {
            ...
            modified_proof[this->prng.template UniformInt<size_t>(0, modified_proof.size() - 1)] ^= std::byte(1);
        ...
        TEST_F(TestAirStarkTest, ChangeAir) {
            ...
            // Modify AIR.
            this->SetAir(
                TestAir(this->trace_length, this->res_claim_index, this->claimed_res + BaseFieldElement::One()));
