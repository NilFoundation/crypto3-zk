#include <nil/crypto3/zk/snark/systems/stark/composition_oracle.hpp>
...
namespace starkware {
    namespace {
        ...
        */
        ...
        public:
            CompositionOracleProverTester(size_t trace_length, size_t n_cosets, size_t n_columns,
                                          bool with_composition_trace) :
                trace_length(trace_length),
                n_cosets(n_cosets), n_columns(n_columns), with_composition_trace(with_composition_trace),
                n_traces(with_composition_trace ? 2 : 1), evaluation_domain(trace_length, n_cosets) {
                // Create trace mocks.
        ...
        */
        void CompositionOracleProverTester::TestEvalComposition(size_t degree_bound) {
            ...
            for (uint64_t i = 0; i < n_cosets; ++i) {
                coset_offsets_bit_reversed.emplace_back(evaluation_domain.CosetOffsets()[BitReverse(i, log_cosets)]);
            }
            ...
            if (with_composition_trace) {
                composition_trace_lde = lde_random_trace(composition_trace_ptr, ExtensionFieldElement::Uninitialized());
            }

            // Composition polynomial mock.
            StrictMock<CompositionPolynomialMock> composition_polynomial;
            EXPECT_CALL(composition_polynomial, GetDegreeBound()).WillRepeatedly(Return(degree_bound * trace_length));
            ...
            CompositionOracleProver oracle_prover(UseOwned(&evaluation_domain), std::move(trace_ptr),
                                                  std::move(composition_trace_ptr), mask, nullptr,
                                                  UseOwned(&composition_polynomial), &channel);

            std::vector<ExtensionFieldElement> result = oracle_prover.EvalComposition(task_size, degree_bound);
        ...
        TEST(CompositionOracleProver, EvalComposition) {
            ...
            // Test no cosets.
            EXPECT_ASSERT(CompositionOracleProverTester(16, 0, 7, true).TestEvalComposition(2),
        ...
        */
        void CompositionOracleProverTester::TestDecommitQueries() {
            ...
            auto create_mask = [&](size_t trace_i, const auto &trace) {
                ...
                EXPECT_CALL(*trace,
                            DecommitQueries(Property(&gsl::span<const std::tuple<uint64_t, uint64_t, size_t>>::size,
                                                     masks_items_in_trace)));
            };
            ...
            oracle_prover.DecommitQueries(
                {{prng.UniformInt<uint64_t>(0, n_cosets - 1), prng.UniformInt<uint64_t>(0, n_traces * n_columns - 1)}});
        }
        ...
        TEST(CompositionOracleProver, DecommitQueries) {
            ...
            // Test no cosets.
            EXPECT_ASSERT(CompositionOracleProverTester(16, 0, 7, true).TestDecommitQueries(),
                          HasSubstr("must be a power of 2"));
        }
        ...
        void CompositionOracleProverTester::TestInvalidMask() {
            ...
            EXPECT_ASSERT(CompositionOracleProver(UseOwned(&evaluation_domain), std::move(trace_ptr), nullptr, mask,
        ...
        void VerifierTestVerifyDecommitment(size_t trace_length, size_t n_cosets, size_t n_columns,
                                            bool with_composition_trace, size_t n_queries) {
            ...
            std::unique_ptr<StrictMock<CommittedTraceVerifierMock<ExtensionFieldElement>>> composition_trace_ptr;
            if (with_composition_trace) {
            ...
            auto create_mask = [&](size_t trace_i, const auto &trace, const auto &uninitialized_element) {
                ...
                EXPECT_CALL(*trace,
                            VerifyDecommitment(Property(&gsl::span<const std::tuple<uint64_t, uint64_t, size_t>>::size,
            ...
            for (size_t i = 0; i < n_queries - 1; ++i) {
                queries.emplace_back(prng.UniformInt<uint64_t>(0, n_cosets - 1),
                                     prng.UniformInt<uint64_t>(0, trace_length - 1));
            }
            // Force a duplicate.
            ...
            EXPECT_CALL(composition_polynomial,
                        EvalAtPoint(testing::Matcher<const BaseFieldElement &>(_), BeginEndDistanceIs(first_mask_size),
            ...
            CompositionOracleVerifier oracle_verifier(UseOwned(&evaluation_domain),
                                                      TakeOwnershipFrom(std::move(trace_ptr)),
                                                      TakeOwnershipFrom(std::move(composition_trace_ptr)), mask,
                                                      nullptr, UseOwned(&composition_polynomial), &channel);
