...
namespace starkware {
    namespace oods {
        ...
        */
        std::pair<CompositionTrace, Coset>
            BreakCompositionPolynomial(gsl::span<const ExtensionFieldElement> composition_evaluation, size_t n_breaks,
                                       const Coset &domain);
        ...
        */
        std::unique_ptr<Air> CreateBoundaryAir(
            ...
            std::vector<std::tuple<size_t, ExtensionFieldElement, ExtensionFieldElement>> &&boundary_constraints);
        ...
        */
        std::vector<std::tuple<size_t, ExtensionFieldElement, ExtensionFieldElement>>
            ProveOods(ProverChannel *channel, const CompositionOracleProver &original_oracle,
        ...
        */
        std::vector<std::tuple<size_t, ExtensionFieldElement, ExtensionFieldElement>>
            VerifyOods(const EvaluationDomain &evaluation_domain, VerifierChannel *channel,
    ...
    namespace oods {
        ...
        std::vector<std::tuple<size_t, ExtensionFieldElement, ExtensionFieldElement>>
            ...
            std::vector<std::tuple<size_t, ExtensionFieldElement, ExtensionFieldElement>> boundary_constraints;
            const ExtensionFieldElement point = channel->GetRandomFieldElementFromVerifier("Evaluation point");
            ...
            // constraints.
            {
                ...
                for (size_t i = 0; i < trace_evaluation_at_mask.size(); ++i) {
                    ...
                    if (!cols_seen[column_index]) {
                        ...
                        boundary_constraints.emplace_back(column_index, conj_point * row_element,
            ...
            // constraints.
            {
                ...
                for (size_t i = 0; i < broken_evaluation.size(); ++i) {
                    ...
                    boundary_constraints.emplace_back(original_oracle.Width() + i, point_transformed,
        ...
        std::vector<std::tuple<size_t, ExtensionFieldElement, ExtensionFieldElement>>
            ...
            for (size_t i = 0; i < mask.size(); ++i) {
                ...
                if (!cols_seen[column_index]) {
                    ...
                    boundary_constraints.emplace_back(column_index, conj_point * Pow(trace_gen, row_offset),
                                                      conj_trace_eval_at_idx_value);
                }
            }
            ...
            const ExtensionFieldElement trace_side_value =
                original_oracle.GetCompositionPolynomial().EvalAtPoint(point, original_oracle_mask_evaluation, {});
            ...
            // evaluation.
            const ExtensionFieldElement broken_side_value = poly_break.EvalFromSamples(broken_evaluation, point);

            // Verify the OODS equation is satisfied.
            ASSERT_RELEASE(trace_side_value == broken_side_value, "Out of domain sampling verification failed.");
