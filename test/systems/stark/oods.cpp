    ...
    namespace {
        ...
        */
        TEST(OutOfDomainSampling, ProveOods) {
            ...
            const auto expected_mask_eval_values = prng.RandomFieldElementVector<ExtensionFieldElement>(mask_size);
            ...
            CompositionOracleProver oracle(UseOwned(&evaluation_domain), UseOwned(&original_trace), nullptr, mask,
                                           nullptr, nullptr, &prover_channel);
        ...
        */
        TEST(OutOfDomainSampling, VerifyOods) {
            ...
            const ExtensionFieldElement expected_point = fake_prover_channel.GetRandomFieldElementFromVerifier("");
            Prng prng;
            const EvaluationDomain evaluation_domain(trace_length, n_cosets);
            ...
            const auto fake_mask_eval_values = prng.RandomFieldElementVector<ExtensionFieldElement>(mask_size);
            const auto fake_composition_trace_values = prng.RandomFieldElementVector<ExtensionFieldElement>(n_breaks);

            // Compute the value of the RHS of the OODS equation.
            ExtensionFieldElement expected_check_value = HornerEval(expected_point, fake_composition_trace_values);
            ...
            EXPECT_CALL(composition_polynomial_mock, GetDegreeBound()).WillRepeatedly(Return(trace_length * n_breaks));

            // Create composition oracle.
            CompositionOracleVerifier oracle(UseOwned(&evaluation_domain), UseOwned(&original_trace), nullptr, mask,
                                             nullptr, UseOwned(&composition_polynomial_mock), &verifier_channel);

            // Mocks returned value on the LHS of the OODS equation, to be the same as the RHS we computed
            // before.
            EXPECT_CALL(composition_polynomial_mock, EvalAtPoint(expected_point, gsl::make_span(fake_mask_eval_values),
        ...
        TEST(OutOfDomainSampling, EndToEnd) {
            ...
            const BaseFieldElement claimed_res = TestAir::PublicInputFromPrivateInput(secret, res_claim_index);
            ...
            CommittedTraceProver<BaseFieldElement> trace_prover(UseOwned(&evaluation_domain), n_columns,
                                                                base_table_prover_factory);
            trace_prover.Commit(TestAir::GetTrace(secret, trace_length, res_claim_index),
                                evaluation_domain.TraceDomain(), true);
            ...
            // Create OracleProver.
            const CompositionOracleProver oracle_prover(UseOwned(&evaluation_domain), UseOwned(&trace_prover), nullptr,
                                                        test_air.GetMask(), UseOwned(&test_air),
                                                        UseOwned(composition_polynomial.get()), &prover_channel);
            ...
            CommittedTraceProver<ExtensionFieldElement> composition_prover(UseOwned(&evaluation_domain), n_breaks,
                                                                           extension_table_prover_factory);
            {
                auto &&[uncommitted_composition_trace, composition_trace_domain] =
                    oods::BreakCompositionPolynomial(composition_polynomial_eval, n_breaks, composition_eval_domain);
                composition_prover.Commit(std::move(uncommitted_composition_trace), composition_trace_domain, false);
            }
            ...
            CommittedTraceVerifier<BaseFieldElement> trace_verifier(UseOwned(&evaluation_domain), n_columns,
            ...
            const CompositionOracleVerifier oracle_verifier(UseOwned(&evaluation_domain), UseOwned(&trace_verifier),
                                                            nullptr, test_air.GetMask(), UseOwned(&test_air),
                                                            UseOwned(composition_polynomial.get()), &verifier_channel);

            // VerifyOods.
            const auto verifier_boundary_constraints =
                oods::VerifyOods(evaluation_domain, &verifier_channel, oracle_verifier, composition_eval_domain);
