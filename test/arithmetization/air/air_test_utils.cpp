...
namespace starkware {
    namespace {
        ...
        TEST(AirTestUtils, ComputeCompositionDegree) {
            ...
            const std::vector<BaseFieldElement> v_rand1 = prng.RandomFieldElementVector<BaseFieldElement>(trace_length);
            std::vector<BaseFieldElement> v_rand1_sqr;
            v_rand1_sqr.reserve(v_rand1.size());
            for (const auto &e : v_rand1) {
                v_rand1_sqr.push_back(e * e);
            }
            const std::vector<BaseFieldElement> v_rand2 = prng.RandomFieldElementVector<BaseFieldElement>(trace_length);
            ...
            // Verify degrees.
            EXPECT_EQ(ComputeCompositionDegree(air, good_trace, random_coefficients),
                      air.GetCompositionPolynomialDegreeBound() - 1);
            EXPECT_EQ(ComputeCompositionDegree(air, bad_trace, random_coefficients),
